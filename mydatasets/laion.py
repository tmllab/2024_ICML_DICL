...
class CsvDataset(Dataset):
    def __init__(self,
        ...
        self.allclassnames = df[name_key].tolist()
        self.labels = list(map(int, df[label_key].tolist()))

        self.classnames = get_classnames('openai')
        self.n_classes = len(np.unique(np.array(self.labels)))
    ...
    def __getitem__(self, idx):
        image = self.transforms(images=Image.open(str(self.images[idx])), text=None)
        # text = self.transforms(images=None, text=[str(self.captions[idx])])
        text = str(self.captions[idx])
        classname = self.allclassnames[idx]
...
def get_csv_dataset(args, preprocess_fn, is_train, epoch=0):
    ...
    # from models.utils import get_world_size, get_rank
    # sampler = DistributedSampler(dataset, num_replicas=get_world_size(),rank=get_rank()) if args.distributed and is_train else None
    sampler = None
...
def get_data(args, preprocess_fns, epoch=0):
    ...
    data["train_ft"] = get_dataset_fn(args.ft_data, args.dataset_type)(args,
                            preprocess_train,
                            is_train=True,
                            epoch=epoch)
...
def get_trainset(args, preprocess, is_train=False):

    dataset = CsvDataset(args.data_path,
                         preprocess,
                         img_key='filepath',
                         caption_key='title',
                         name_key='classname',
